app.admin.resolutions.show.menu = (router, resolution) => (a,x) => {
  ...
  let render = (router, selected) => {
    ...
    return [
      ...
      validResolutionDivisions.map((division) => app.button({
        ...
        onclick: () => router.open(`/admin/resolutions/${router.params.resolution_id}/${division}`),
        class: 'btn app-btn d-block w-100 text-left',
        buttonTag: {
          $init: (el) => {
      ...
      addableDivisions.length ? app.form({
        form: f => [
          f.select({
            ...
            selectTag: {
              $on: {
                'input': (e, el) => router.open(`/admin/resolutions/${router.params.resolution_id}/${el.value}`)
              },
            }
          })
        ]
