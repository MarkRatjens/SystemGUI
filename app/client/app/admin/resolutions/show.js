        ...
        $nodes: (el, resolution) => [
          app.float({
            left: app.button({
              ...
              onclick: () => router.open(`/admin/resolutions/${router.params.resolution_id}/json`),
              class: "btn btn-outline-info",
            }),
            right: [
              app.button({
                label: app.icon("fas fa-map", "Blueprint"),
                onclick: () => router.open(`/admin/blueprints/${router.params.resolution_id}`),
              }),
              app.button({
                label: app.icon("fas fa-box-open", "Pack"),
                onclick: () => router.open(`/admin/packing/${router.params.resolution_id}`),
              }),
            ],
          }),
          a.hr,
          a['div.row']([
            a['div.col-3'](app.admin.resolutions.show.menu(router, resolution)),
            a['div.col-9'](app.admin.resolutions.show.body(router, resolution)),
          ]),
          a.hr,
          app.float({
            right: [
