app.docker.images.image = (image) =>
a['app-docker-images-image.app-dashboard-item']({
  id: `app-docker-images-image-${image.identifier}`,
  ...
  $reindex: (el) => (event) => {
    el.$('app-docker-images-state').$reindex(image)
  },
  $receive: (el) => (event) => {
    el.$('app-docker-images-state').$restate(event.payload, event.action)
    if (el.$('button[data-name=log]').classList.contains('active')) {
      if (event.action == 'start') {
        el.$('app-docker-images-log').$restart()
      }
